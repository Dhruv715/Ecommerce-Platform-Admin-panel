  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/AllOrders', {
          headers: {
            'auth': token
          }
        });
    ...
    const fetchUsers = async () => {
      try {
        const token = localStorage.getItem('Token');
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/AllUsers', {
          headers: {
            'auth': token
          }
        });
    ...
    const fetchProducts = async () => {
      try {
        const token = localStorage.getItem('Token');
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/AllProduct', {
          headers: {
            'auth': token
          }
        });
                  ...
                  <TableCell>
                    {products[order.productId] ? (
                      <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                        <img
                          src={`https://ecommerce-platform-kfby.onrender.com/images/${products[order.productId].image[0]}`}
