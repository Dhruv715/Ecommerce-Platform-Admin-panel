...
function Sellers() {
  ...
  useEffect(() => {
    const fetchSellers = async () => {
      try {
        ...
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/AllSellers', {
          headers: {
  ...
  const handleStatusChange = async (sellerId, newStatus) => {
    try {
      ...
      const response = await axios.post(`https://ecommerce-platform-kfby.onrender.com/Admin/updateSellerStatus/${sellerId}`, { status: newStatus }, {
        headers: {
          'auth': token
        }
      });
