...
import { Box, Typography, Container, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Button, MenuItem, Select, FormControl, InputLabel } from '@mui/material';
...
function SubCategory() {
    ...
    const fetchSubCategories = async () => {
      try {
        ...
        const [subCategoryResponse, categoryResponse] = await Promise.all([
          axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/SubCategory', {
            headers: { 'auth': token } // Include the token in the headers
          }),
          axios.get('https://ecommerce-platform-kfby.onrender.com/Admin/Category', {
            headers: { 'auth': token } // Include the token in the headers
          })
        ]);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await axios.put(`https://ecommerce-platform-kfby.onrender.com/Admin/updateSubcategory/${selectedSubCategory._id}`, updatedSubCategory, {
        headers: {
          'auth': token
        }
      });
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('Token');
      const response = await axios.delete(`https://ecommerce-platform-kfby.onrender.com/Admin/deleteSubcategory/${id}`, {
        headers: {
          'auth': token
        }
      });
            ...
            <TableBody>
              {subCategories.map((subCategory) => (
                <TableRow key={subCategory._id}>
                  <TableCell>
                    <img
                      src={`https://ecommerce-platform-kfby.onrender.com/images/${subCategory.image}`}
